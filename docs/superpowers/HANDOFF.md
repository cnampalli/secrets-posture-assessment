...
>
> **State:** Phases 0 / 0.5 / 1(slices 1–2) merged to `main`. **Phase 1 remainder + Phase 2 — DONE and
> MERGED to `main`** (PR #15, merge commit `42a1e75`, 2026-06-05). The work delivers: per-domain
...
>
> **Next session:** the open follow-ups in the roadmap (PR #15 is merged; start from `main`):
> Phase 1 #4 (single-pass token escaping) / #8 (Domain-from-YAML); PR-review follow-ups
...
>
> **Recoverable anywhere:** everything is merged + pushed to `main`. On another machine: clone the repo,
> then `git pull` (you're on `main`). The claude-mem observation
